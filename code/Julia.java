...
class Julia extends Canvas{
	...
	public Julia(double c_re, double c_im,int iterations) { 
	...
	public Julia(double c_re, double c_im) { 
	...
	// call paint from parent class 
		...
		for (int row = 0; row < 800; row++) {
				for (int col = 0; col < 800; col++) {
					...
					double x=(((double)col*(xmax-xmin))/800)+xmin;
					double y=(((double)row*(ymax-ymin))/800)+ymin;
					
					
					//checking the condition for a complex number z to make the julia set(i.e. Zn+1 = Zn2+C but  Z0 is any complex number within area of interest(fixed) and c contant,where ABS(Zn) > 2 for given iterations)
					int iteration = 0;
